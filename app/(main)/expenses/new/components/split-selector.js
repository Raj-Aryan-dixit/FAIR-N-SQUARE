...
export function SplitSelector({
  ...
  const updatePercentageSplit = (userId, newPercentage) => {
    // Cap the percentage to ensure the amount doesn't exceed the total
    const maxPercentage = 100;
    const cappedPercentage = Math.min(Math.max(0, newPercentage), maxPercentage);
    
    // Update just this user's percentage and recalculate amount
    const updatedSplits = splits.map((split) => {
      if (split.userId === userId) {
        const calculatedAmount = (amount * cappedPercentage) / 100;
        return {
          ...split,
    ...
    <div className="space-y-4 mt-4">
      ...
      {type === "percentage" && !isPercentageValid && (
        <div className="text-sm text-amber-600 mt-2">
          The percentages should add up to 100%. Current total: {totalPercentage.toFixed(1)}%
        </div>
      )}
      
      {type === "percentage" && totalAmount > amount && (
        <div className="text-sm text-red-600 mt-2">
          Total split amount (${totalAmount.toFixed(2)}) exceeds the expense amount (${amount.toFixed(2)}). Please adjust the percentages.
        </div>
      )}
